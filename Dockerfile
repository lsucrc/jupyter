...
RUN mkdir -p -m 700 /root/.jupyter/custom && \
    ...
    echo "#ipython_notebook img{display:block; background: url(logo.png) no-repeat; background-size: contain; width: 233px; height: 33px; padding-left: 233px; -moz-box-sizing: border-box; box-sizing: border-box;
}" >> /root/.jupyter/custom/custom.css 

ADD https://github.com/lsucrc/notebook/blob/master/logo.png /root/.jupyter/custom/logo.png
